...
export const createLectureSummary = asyncHandler(
...
export const updateLectureSummary = asyncHandler(
  async (req: Request, res: Response) => {
    const summary = await LectureSummary.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true },
    );
...
export const deleteLectureSummary = asyncHandler(
  async (req: Request, res: Response) => {
...
export const getCourseSummaries = asyncHandler(
  async (req: Request, res: Response) => {
    const summaries = await LectureSummary.find({
...
export const getSummary = asyncHandler(async (req: Request, res: Response) => {
  const summary = await LectureSummary.findById(req.params.id).populate(
    "course",
  );
  if (!summary) throw ApiError.notFound("Lecture summary not found");
  sendSuccess(res, summary);
...
export const getMySummaries = asyncHandler(
  async (req: Request, res: Response) => {
