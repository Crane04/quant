...
export const createAnnouncement = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const announcement = await Announcement.create({
      ...
      course: course?._id,
      timetableSlot:
        body.type === "lecture_alert" ? body.timetableSlotId : undefined,
...
export const getUnsent = asyncHandler(async (_req: Request, res: Response) => {
  const announcements = await Announcement.find({ sentAt: null })
    .populate("course")
    .sort({ createdAt: 1 });
  sendSuccess(res, announcements);
});
...
export const listAllAnnouncements = asyncHandler(
  async (req: Request, res: Response) => {
    const { type, university, department, level } = req.query as Record<
      string,
      string
    >;
