...
export default function OverviewPage() {
  ...
  const totalDownloads = docs.reduce(
    (sum: number, d: PDFDoc) => sum + d.downloadCount,
    0,
  );
  const uniqueCourses = new Set(
    docs.map((d: PDFDoc) => d.course?.code).filter(Boolean),
  ).size;
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-sm text-zinc-500 mt-1">Global material library</p>
      </div>
                ...
                <div key={doc._id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-zinc-500 font-mono">
                      {doc.course?.code || "Unknown course"}
                    </p>
                  </div>
                </div>
              ))
