...
{
  "intent": "greeting" | "thanks" | "goodbye" | "help" | "menu" | "set_assignment" | "find_document" | "search_documents" | "select_document" | "unsupported_request" | "unknown",
...
- Use "set_assignment" when the user wants to add, save, schedule, or be reminded about an assignment, test, quiz, lab, report, project, deadline, or coursework task.
...
You extract assignment reminder details for a university WhatsApp bot.
Return only valid JSON with these fields:
{
  "courseCode": string | null,
...
Rules:
- Extract courseCode from messages like "mee305", "MEE 305", "phy201", "GST 101"; normalize with a space and uppercase letters, e.g. "mee305" -> "MEE 305".
- description should be the actual task, e.g. "lab report", "tutorial 2", "thermodynamics assignment". Keep it short but meaningful.
- dueDate must be ISO format YYYY-MM-DD.
- Use the provided current date to resolve relative dates like tomorrow, next Friday, this evening, end of month.
- If a field is genuinely missing, return null for that field.
- Do not invent a course code, description, or date.
`;
...
const parseAssignmentDetails = (content: string): AssignmentDetails | null => {
  try {
    const parsed = JSON.parse(extractJson(content)) as Partial<AssignmentDetails>;

    return {
      courseCode:
        typeof parsed.courseCode === "string" && parsed.courseCode.trim()
          ? parsed.courseCode.trim().toUpperCase().replace(/([A-Z]+)\s?(\d+)/, "$1 $2")
          : undefined,
      description:
...
export const extractAssignmentDetails = async (
  message: string,
): Promise<AssignmentDetails | null> => {
  const apiKey = process.env.GROQ_API_KEY;

  if (!message.trim()) {
    logWarn("AI assignment extraction skipped: empty message");
    return null;
  }

  if (!apiKey) {
    logWarn("AI assignment extraction skipped: GROQ_API_KEY is not set");
    return null;
  }
  ...
  try {
    ...
    logInfo("AI assignment extraction request started", {
      model,
      message: truncate(message),
    });
    ...
    if (!response.ok) {
      logWarn("AI assignment extraction request failed", {
        status: response.status,
        statusText: response.statusText,
        body: truncate(await response.text()),
      });
    ...
    const data = (await response.json()) as GroqChatResponse;
    const content = data.choices?.[0]?.message?.content;
    const details = content ? parseAssignmentDetails(content) : null;

    if (!details) {
      logWarn("AI assignment extraction returned invalid payload", {
        content: truncate(content || ""),
      });
      return null;
    }

    logInfo("AI assignment extraction parsed", {
    ...
    const message = err instanceof Error ? err.message : "Unknown Groq error";
    logWarn("AI assignment extraction unavailable", { error: message });
    return null;
  }
};
