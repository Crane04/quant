...
export const addOrUpdateGrade = asyncHandler(
...
export const deleteGrade = asyncHandler(async (req: Request, res: Response) => {
  const record = await GradeRecord.findOneAndDelete({
    _id: req.params.id,
...
export const getMyCgpa = asyncHandler(async (req: Request, res: Response) => {
  const records = await GradeRecord.find({ student: req.studentId }).populate(
    "course",
  );
  ...
  const semesterBreakdown = Array.from(bySemester.entries()).map(
    ([key, v]) => ({
      session: key.split(" - ")[0],
