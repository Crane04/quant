...
export async function ensureDefaultBadges(): Promise<void> {
  for (const badge of DEFAULT_BADGES) {
    await Badge.findOneAndUpdate({ key: badge.key }, badge, {
      upsert: true,
      new: true,
    });
  }
...
async function meetsCriteria(
  ...
  switch (badge.criteriaType) {
    ...
    case "past_question_upload_count":
      return (
        stats.approvedPastQuestionCount >= (badge.criteriaValue ?? Infinity)
      );
    case "streak_days":
      return stats.uploadStreakDays >= (badge.criteriaValue ?? Infinity);
    case "department_rank":
      return (
        stats.departmentRank !== null &&
        stats.departmentRank <= (badge.criteriaValue ?? 1)
      );
    case "global_rank":
      return (
        stats.globalRank !== null &&
        stats.globalRank <= (badge.criteriaValue ?? 1)
      );
    ...
    case "lifetime_points_ambassador":
      return (
        stats.isAmbassador &&
        stats.lifetimePointsEarned >= (badge.criteriaValue ?? Infinity)
      );
    case "download_count":
 ...
 */
export async function evaluateBadgesForStudent(
  studentId: string,
): Promise<BadgeDoc[]> {
