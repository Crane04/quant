 ...
 */
export async function requireAmbassador(req: Request, _res: Response, next: NextFunction) {
  const student = await Student.findById(req.studentId);
  if (!student) return next(ApiError.notFound("Student not found"));
  if (!student.isAmbassador) {
    return next(ApiError.forbidden("The HOC Hub is available to ambassadors only"));
  }
