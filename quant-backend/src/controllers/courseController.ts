...
export const listCourses = asyncHandler(async (req: Request, res: Response) => {
  const { university, department, level, session, semester, search } = req.query;
...
export const enrollInCourses = asyncHandler(async (req: Request, res: Response) => {
  ...
  const enrollments = await StudentCourse.find({ student: req.studentId, session, semester }).populate(
    "course"
  );
...
export const getMyCourses = asyncHandler(async (req: Request, res: Response) => {
  ...
  const enrollments = await StudentCourse.find(filter).populate("course").sort({ createdAt: -1 });
